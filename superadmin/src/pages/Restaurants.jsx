...
export default function Restaurants() {
  ...
  const [modal, setModal] = useState(null); // null | 'add' | venue object
  ...
  const save = async (e) => {
    ...
    try {
      if (modal === 'add') {
        await api.post('/restaurants/admin', { ...form, type: 'restaurant' });
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={row}>
        <div>
          <div style={title}>Restaurants</div>
          <div style={sub}>Manage all restaurants on the platform.</div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button style={btnOutline}><Download size={13} style={{ marginRight: 4 }} />Export</button>
          <button style={btnPrimary} onClick={() => { setForm(EMPTY); setModal('add'); }}><Plus size={13} style={{ marginRight: 4 }} />Add Restaurant</button>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ position: 'relative', marginLeft: 'auto' }}>
          ...
          <input placeholder="Search..." onChange={e => setFilters(f => ({ ...f, search: e.target.value, page: 1 }))}
            style={{ paddingLeft: 32, ...inputStyle, width: 200 }} />
        </div>
      </div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr style={{ borderBottom: '1px solid #e2e8f0' }}>
              {['Restaurant', 'Location', 'Cuisine', 'Status', 'Rating', 'Orders', 'Revenue', 'Actions'].map(h => (
                <th key={h} style={{ textAlign: 'left', padding: '10px 12px', fontSize: 11, color: '#a0aec0', fontWeight: 600 }}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {loading ? <tr><td colSpan={8} style={emptyCell}>Loading...</td></tr>
                : data.venues.length === 0 ? <tr><td colSpan={8} style={emptyCell}>No restaurants found</td></tr>
                  : data.venues.map(v => (
                    <tr key={v._id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                      <td style={{ padding: '10px 12px', fontWeight: 600, color: '#1a1a2e', fontSize: 13 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ fontSize: 18 }}>🍽️</span>{v.name}
                        </div>
                      </td>
                      <td style={cellMuted}>{v.location?.city}{v.location?.country ? `, ${v.location.country}` : ''}</td>
                      <td style={cellMuted}>{v.cuisine || '—'}</td>
                      ...
                      <td style={{ padding: '10px 12px' }}>
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button style={iconBtn} onClick={() => toggleStatus(v)} title={v.status === 'active' ? 'Deactivate' : 'Activate'}><Eye size={13} /></button>
        ...
        <div style={overlay} onClick={() => setModal(null)}>
          <div style={modalBox} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
              <div style={{ fontWeight: 700, fontSize: 16 }}>{modal === 'add' ? 'Add Restaurant' : 'Edit Restaurant'}</div>
            ...
            <form onSubmit={save} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <div><label style={labelStyle}>Cuisine</label><input style={inputStyle} value={form.cuisine} onChange={e => setForm(f => ({ ...f, cuisine: e.target.value }))} /></div>
              ...
              <div style={{ gridColumn: 'span 2' }}>
                ...
                <select style={inputStyle} value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value }))}>
                  <option value="pending">Pending</option><option value="registered">Registered</option><option value="inactive">Inactive</option>
                </select>
              </div>
              {modal === 'add' && <>
                <div style={{ gridColumn: 'span 2', borderTop: '1px solid #e2e8f0', paddingTop: 14, marginTop: 4 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: '#718096', marginBottom: 10 }}>RESTAURANT ADMIN ACCOUNT (optional)</div>
                </div>
                <div><label style={labelStyle}>Admin Name</label><input style={inputStyle} placeholder="Owner's name" value={form.adminName} onChange={e => setForm(f => ({ ...f, adminName: e.target.value }))} /></div>
                <div><label style={labelStyle}>Admin Email</label><input style={inputStyle} type="email" placeholder="owner@example.com" value={form.adminEmail} onChange={e => setForm(f => ({ ...f, adminEmail: e.target.value }))} /></div>
...
const title = { fontSize: 20, fontWeight: 700, color: '#1a1a2e' };
